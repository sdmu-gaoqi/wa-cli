...
const server = async(filePath, argv) => {
  ...
  try {
    ...
    if (https === 'true') {
      ...
      console.log(cert || resolve(currentFilePath, './ssl/cert.pem'), 'certcertcertcertcertcertcertcert');
      args.push('-S', '-C', `${cert || resolve(currentFilePath, './ssl/cert.pem')}`, '-K', `${private || resolve(currentFilePath, './ssl/key.pem')}`);
    }
